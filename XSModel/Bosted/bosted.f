...
c
      ...
      Q2=4.*E*Ep*(sin(abs(theta)/2.))**2.
      w2=M**2.+2.*M*nu-Q2

      call F1F2IN09(Z,A,q2,w2,F1,F2,r)

      xs1=(2./137.*Ep/q2*cos(theta/2))**2. !mott
      xs1=xs1*(2/M*F1*tan(abs(theta)/2)**2+F2/nu)
      xs1=xs1*389.379
      ...
      bostedxs=(xs1+xs2)/1000.    !ub/MeV-sr
C      write(*,*) xs1,xs2
      return
...
c new 5/07 values. Values 1 and 7 will be overridden below.
     ...
     >  q2.lt.0.0 .or. q2.gt.11.0) then
        write(6,'(1x,''error, q2 or w2 out of range'',2f8.3)') q2,w2
        return
      endif
